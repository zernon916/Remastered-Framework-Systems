...
local IDENTITY_TAG_EVERY = 80 -- 2 s nametag RPC; unit think used to push every ready tick
-- Far hostiles: do not coveringBeacon every unit every tick (map-wide totebot lag).
local FAR_SCAN_SLACK = 8
local DISCOVER_EVERY_TICKS = 40 -- 1 s new-hostile scan; pending/DROP still every auto tick
local FAR_DROP_MUL = 2 -- immediately DROP when farther than 2× device range
...
-- convertingOnly=true: the beacon that converted this ally, never another powered device.
function RfsBotHijack.coveringBeacon( unit, fullRange, convertingOnly )
	if not unit or not sm.exists( unit ) or not unit.character or not sm.exists( unit.character ) then
		return nil
	end
	...
	local info = key and RfsBotHijack.allies[key]
	if info then
		-- convertingOnly=true: tethered allies should follow the beacon that held them,
		-- not the "sticky" converting device key used for cap bookkeeping.
		if convertingOnly then
			if info.mode ~= "infected" and info.beaconKey then
				preferred = tostring( info.beaconKey )
			end
		else
			if info.hackBeaconKey then
				preferred = tostring( info.hackBeaconKey )
			elseif info.mode ~= "infected" and info.beaconKey then
				preferred = tostring( info.beaconKey )
			end
		end
	end
...
-- Cheap field test: skip coveringBeacon on totebots far from every live device.
local function poweredBeaconFields()
	local now = 0
	pcall( function()
		now = sm.game.getCurrentTick()
	end )
	if RfsBotHijack._fieldTick == now and type( RfsBotHijack._fieldCache ) == "table" then
		return RfsBotHijack._fieldCache
	end
	local fields = {}
	for _, rec in pairs( RfsBotHijack.beacons or {} ) do
		if rec and rec.pos and beaconLive( rec ) then
			local r = tonumber( rec.range ) or DEFAULT_RANGE
			if r < 1 then
				r = DEFAULT_RANGE
			end
	...
	RfsBotHijack._fieldTick = now
	RfsBotHijack._fieldCache = fields
	return fields
end

local function unitNearPoweredBeacon( unit )
	if not unit or not unit.character or not sm.exists( unit.character ) then
		return false
	end
	local fields = poweredBeaconFields()
	...
	for i = 1, #fields do
		local f = fields[i]
		local okWorld = true
		if world and f.world then
			okWorld = sameWorld( world, f.world )
		end
		if okWorld then
			local d2 = ( f.pos - pos ):length2()
			if d2 <= f.r2 then
				return true
			end
		end
...
function RfsBotHijack.revert( unit )
	...
	if type( RfsHackApply ) == "table" and type( RfsHackApply.clearPublicApply ) == "function" then
	...
	if type( RfsHackOrdersDrop ) == "table" and type( RfsHackOrdersDrop.afterUnhack ) == "function" then
		pcall( RfsHackOrdersDrop.afterUnhack, unit, info )
	end
end
...
function RfsBotHijack.prune( world )
	...
	for key, info in pairs( RfsBotHijack.allies ) do
		if live[key] then
			...
			gone[#gone + 1] = { key = key, info = info }
		end
	end
	RfsBotHijack.allies = keep
	if #gone > 0 and type( RfsHackOrdersDrop ) == "table" and type( RfsHackOrdersDrop.afterGoneMany ) == "function" then
		pcall( RfsHackOrdersDrop.afterGoneMany, gone )
	end
...
function RfsBotHijack.tick( world )
	...
	for key, info in pairs( RfsBotHijack.allies ) do
		local unit = live[key]
		local dead = false
		if unit and type( RfsHackOrdersDrop ) == "table" and type( RfsHackOrdersDrop.unitLooksDead ) == "function" then
			pcall( function()
				dead = RfsHackOrdersDrop.unitLooksDead( unit ) and true or false
			end )
		end
		if not unit or dead then
			drop[#drop + 1] = { key = key, info = info }
		else
			...
			elseif info.mode == "infected" then
				...
				-- Tethered: converting beacon only. Do not hop onto another powered device.
				local rec, bkey = RfsBotHijack.coveringBeacon( unit, true, true )
					...
					if rec.canInfect then
						...
						if info.infectAcc >= need then
							...
							if unit.character and sm.exists( unit.character ) then
								-- Convert-to-infect tint once; paint gun must be able to override later.
	...
	if #drop > 0 then
		if type( RfsHackOrdersDrop ) == "table" and type( RfsHackOrdersDrop.afterGoneMany ) == "function" then
			pcall( RfsHackOrdersDrop.afterGoneMany, drop )
		else
			for _, rec in ipairs( drop ) do
				if rec and rec.key then
					RfsBotHijack.allies[rec.key] = nil
				end
			end
		end
	end
...
local function closestOtherRobotCharacter( selfUnit, wantAlly, maxRange )
	...
	local every = 10
	if type( RfsHackAllyThrottle ) == "table" and RfsHackAllyThrottle.AGGRO_EVERY then
		every = tonumber( RfsHackAllyThrottle.AGGRO_EVERY ) or 10
	end
	local slot = unitKey( selfUnit ) .. ( wantAlly and ":a" or ":h" )
	RfsBotHijack._aggroCache = RfsBotHijack._aggroCache or {}
	local hit = RfsBotHijack._aggroCache[slot]
	if hit and ( now - ( hit.tick or 0 ) ) < every then
		if hit.char and sm.exists( hit.char ) then
			return hit.char
		end
	...
	RfsBotHijack._aggroCache[slot] = { tick = now, char = best }
	return best
end
...
function RfsBotHijack.pushTag( unit, text, kind )
	...
	-- Name tags: skip identical RPC storms. HACK/DROP/CHAIN still send every call.
	if kind == "name" and text ~= "" then
		local now = 0
		pcall( function()
			now = sm.game.getCurrentTick() or 0
		end )
		local every = IDENTITY_TAG_EVERY
		if type( RfsHackAllyThrottle ) == "table" and RfsHackAllyThrottle.TAG_EVERY then
			every = tonumber( RfsHackAllyThrottle.TAG_EVERY ) or every
		end
		local key = unitKey( unit )
		RfsBotHijack._tagSent = RfsBotHijack._tagSent or {}
		local prev = key and RfsBotHijack._tagSent[key]
		if prev and prev.text == text and prev.kind == "name" and ( now - ( prev.tick or 0 ) ) < every then
			return
		end
		if key then
			RfsBotHijack._tagSent[key] = { text = text, kind = "name", tick = now }
		end
	end
	pcall( function()
...
function RfsBotHijack.ensureCharHooks()
	...
	local function wrapClientData( cls )
		...
		-- Set before wrapping client_onUpdate so a throw cannot nest wrappers every 40 ticks.
		cls._rfsTagRpc = true
		...
		function cls.sv_e_rfsBotAction( self, params )
			params = params or {}
			local key = params.unitKey and tostring( params.unitKey ) or nil
			local title = params.title and tostring( params.title ) or nil
			if not key or key == "" then
				pcall( function()
					local u = self.character and self.character:getUnit()
					key = u and tostring( u.id ) or nil
			...
			if ( not title or title == "" ) and self.cl and self.cl.rfsLastTag and self.cl.rfsLastTag.text then
				title = tostring( self.cl.rfsLastTag.text )
			end
			pcall( function()
				sm.event.sendToGame( "sv_rfs_botActionOpen", {
					unitKey = key,
					title = title,
					player = params.player,
				} )
		...
		if not cls._rfsRenameInteract then
			local function charClientAlly( self )
				local okAlly = false
				pcall( function()
				...
				pcall( function()
					...
					local pd = char and char.publicData
					if type( pd ) == "table" and pd.rfsPlayerAlly then
						okAlly = true
						if self.cl then
							self.cl.rfsCanRename = true
						end
			...
			local function holdingPaintTool()
				if type( RfsBotInteract ) == "table" and RfsBotInteract.holdingPaintTool then
					return RfsBotInteract.holdingPaintTool()
				end
				return false
			end
			local origCan = cls.client_canInteract
			cls.client_canInteract = function( self )
				if holdingPaintTool() then
					if origCan then
						return origCan( self )
					end
					return false
				end
				if charClientAlly( self ) then
					pcall( function()
						sm.gui.setInteractionText( "", sm.gui.getKeyBinding( "Use", true ), "Commands / Storage" )
					end )
					return true
				end
				if origCan then
			...
			cls.client_onInteract = function( self, character, state )
				...
				if state and charClientAlly( self ) then
					-- E on allied bot: Commands / Storage menu (not rename). Beacon E stays Orders.
					local unitKey = nil
					local title = "BOT"
					pcall( function()
						local u = self.character and self.character:getUnit()
					...
					pcall( function()
						if self.cl and self.cl.rfsLastTag and self.cl.rfsLastTag.text then
							title = tostring( self.cl.rfsLastTag.text )
						end
					end )
					-- Prefer direct Game host open when visible (same process as Orders).
					local opened = false
					...
					if not opened then
						-- Character sandbox → server → Game client (rename-style bounce).
						pcall( function()
							self.network:sendToServer( "sv_e_rfsBotAction", {
								unitKey = unitKey,
								title = title,
		...
		if not cls._rfsTagClientUpdate then
			local origU = cls.client_onUpdate
			cls.client_onUpdate = function( self, dt )
				if origU then
					origU( self, dt )
				end
				-- Re-seat FX only if lost. Do not setCharacterDebugText every frame.
...
function RfsBotHijack.popUnit( unit )
	...
	RfsBotHijack.pushTag( unit, "" )
	local popKey = unitKey( unit )
	local popInfo = popKey and RfsBotHijack.allies[popKey] or nil
	RfsBotHijack.unregister( unit )
	if type( RfsHackOrdersDrop ) == "table" and type( RfsHackOrdersDrop.afterGone ) == "function" then
		pcall( RfsHackOrdersDrop.afterGone, popKey, popInfo )
	end
	if pos then
		pcall( function()
...
function RfsBotHijack.tickAuto( world )
	...
	for key, unit in pairs( live ) do
		...
		if alreadyDone then
			if not unitNearPoweredBeacon( unit ) then
				pcall( function()
		...
		local doDiscover = ( now - ( RfsBotHijack._discoverTick or 0 ) ) >= DISCOVER_EVERY_TICKS
		if doDiscover and not nextPending[key] and not loadGrace and not unitLooksSavedAlly( unit ) and not alreadyDone and not RfsBotHijack.isLockedOut( unit ) and RfsBotHijack.isHackable( unit ) then
			if unit.character and sm.exists( unit.character ) and RfsBotHijack.isRobotCharacter( unit.character ) then
				if unitNearPoweredBeacon( unit )
					and not ( RfsBotHijack.isUndergroundBotCharacter( unit.character ) and not undergroundBotsOn() ) then
					local rec, bkey = RfsBotHijack.coveringBeacon( unit )
					local need = rec and beaconHijackNeed( rec ) or 0
	...
	if ( now - ( RfsBotHijack._discoverTick or 0 ) ) >= DISCOVER_EVERY_TICKS then
		RfsBotHijack._discoverTick = now
	end
				...
				if pos and not RfsBotHijack.areaHasRaid( pos, world ) then
					...
					for hkey, u in pairs( live ) do
						if not claimed[hkey] and sm.exists( u ) and u ~= src and u.character and sm.exists( u.character ) then
							local d2 = ( u.character.worldPosition - pos ):length2()
							if d2 <= maxD2 and ( bestD2 == nil or d2 < bestD2 )
								and unitNearPoweredBeacon( u )
...
local function tetherBeaconRec( info )
	if type( info ) ~= "table" then
		return nil, nil
	end
	local bkey = nil
	-- Tethered bots: same 8/5/3s as the beacon that held them.
	if info.mode ~= "infected" and info.beaconKey ~= nil then
		bkey = tostring( info.beaconKey )
	end
	if not bkey then
		return nil, nil
	end
	local rec = RfsBotHijack.beacons and RfsBotHijack.beacons[bkey]
	return rec, bkey
end

local function shouldRunSignalLoss()
	if type( RfsHackApply ) ~= "table" or type( RfsHackApply.hasLiveBeacon ) ~= "function" then
		return true
	end
	local okLive, liveBeacons = pcall( RfsHackApply.hasLiveBeacon )
	if okLive and liveBeacons then
		return true
	end
	-- When tethered allies are missing a refreshed `info.controlled` (e.g. beacon
	-- destroyed immediately after hijack), we still want to run signal-loss
	-- validation as long as we have beacon records to compare against.
	if RfsBotHijack.beacons and next( RfsBotHijack.beacons ) ~= nil then
		for _, info in pairs( RfsBotHijack.allies or {} ) do
			if info and info.mode ~= "infected" and info.mode ~= "handheld" then
				return true
			end
		end
	end
	for _, info in pairs( RfsBotHijack.allies or {} ) do
		if info and info.controlled and info.mode ~= "infected" then
			return true
		end
	end
	-- Station Core can adopt hijacks without a refreshed `info.controlled` row.
	-- If we know the coverage beacon and hijack timer, still run signal-loss
	-- so DROP countdown can start instead of stalling forever.
	for _, info in pairs( RfsBotHijack.allies or {} ) do
		if info and info.mode ~= "infected" and info.mode ~= "handheld" and info.beaconKey ~= nil then
			local ht = tonumber( info.hijackTicks )
			if ht and ht > 0 then
				return true
			end
		end
...
function RfsBotHijack._tickSignalLoss( live, now )
	...
	if not shouldRunSignalLoss() then
		return
	end
	...
	local toRelease = {}
	for key, info in pairs( RfsBotHijack.allies or {} ) do
		if info and info.mode ~= "infected" and info.mode ~= "handheld" and not info.doomed and not ( RfsBotHijack.jams and RfsBotHijack.jams[key] ) then
			local unit = live[key]
			if unit and sm.exists( unit ) then
				local tetherRec, bkey = tetherBeaconRec( info )
				-- "beaconLive" includes a 1s "power cannot flicker off" hold.
				-- For tethered bots we must also require actual spendable power,
				-- otherwise unpowered beacons can keep the ally hacked.
				local tetherLive = tetherRec and bkey and RfsBotHijack.beaconLive( tetherRec ) or false
				local tetherCanSpend = false
				if tetherLive and tetherRec and type( tetherRec.canSpendOne ) == "function" then
					local ok, can = pcall( tetherRec.canSpendOne )
					tetherCanSpend = ok and can and true or false
				end
				local tetherValid = tetherLive and tetherCanSpend or false
				-- Far from converting device: DROP/release without coveringBeacon (map lag).
				local farImmediate = false
				if unit.character and sm.exists( unit.character ) then
					if tetherRec and tetherRec.pos then
						local r = tonumber( tetherRec.range ) or DEFAULT_RANGE
						if r < 1 then
							r = DEFAULT_RANGE
						end
						local lim = r * FAR_DROP_MUL
						local d2 = ( tetherRec.pos - unit.character.worldPosition ):length2()
						farImmediate = d2 > ( lim * lim )
					elseif not unitNearPoweredBeacon( unit ) then
						farImmediate = true
					end
				end
				-- Tether = converting beacon range + powered (not any beacon in the world).
				local covered = false
				if not farImmediate then
					covered = ( RfsBotHijack.coveringBeacon( unit, false, true ) ~= nil ) and tetherCanSpend
				end
				if covered then
					info.outStreak = 0
					...
					if tetherRec and tetherRec.hijackTicks then
						info.hijackTicks = tetherRec.hijackTicks
					end
					...
					info.inStreak = 0
					info.outStreak = ( farImmediate or not tetherValid ) and OUT_STREAK_TICKS
						or ( ( info.outStreak or 0 ) + 1 )
					info.controlled = true
					local d = RfsBotHijack.drops[key]
	...
	for _, unit in ipairs( toRelease ) do
		...
		-- DROP 8.0 done: unhack (clear ally/hijack) then Orders drop+refresh.
...
local function destVecFromOrder( self )
	...
	if type( order ) == "table" and type( order.dest ) == "table" then
	...
	if not homeKey and self.saved then
		homeKey = self.saved.playerAllyWorkBeacon or self.saved.playerAllyBeacon
	end
	if homeKey then
...
-- Sentry: walk a circle at ~85% of beacon job radius (perimeter patrol).
local SENTRY_WAYPOINTS = 8

function RfsBotHijack.driveSentryPatrol( self, patrolRadius )
	if not self or not self.unit or not sm.exists( self.unit ) then
		return false
	end
	local key = unitKey( self.unit )
	...
	patrolRadius = tonumber( patrolRadius ) or tonumber( rec.range ) or DEFAULT_RANGE
	patrolRadius = math.max( 6, patrolRadius * 0.85 )
	self.saved = self.saved or {}
	local idx = math.floor( tonumber( self.saved.rfsSentryIdx ) or 0 ) % SENTRY_WAYPOINTS
	local function waypoint( i )
		local angle = ( i / SENTRY_WAYPOINTS ) * ( 2 * math.pi )
		return sm.vec3.new(
			center.x + math.cos( angle ) * patrolRadius,
			center.y + math.sin( angle ) * patrolRadius,
			center.z
		)
	end
	local dest = waypoint( idx )
	pcall( function()
		local char = self.unit.character
		if char and sm.exists( char ) then
			if ( char.worldPosition - dest ):length2() <= 9 then
				idx = ( idx + 1 ) % SENTRY_WAYPOINTS
				self.saved.rfsSentryIdx = idx
				self.isDirty = true
				dest = waypoint( idx )
			end
		end
	end )
	return driveToBeaconPos( self, dest, 4 )
end
...
function RfsBotHijack.setOrder( unitOrKey, order, player, allowHost )
	...
	if unit and sm.exists( unit ) and type( RfsBotOrders ) == "table" and type( RfsBotOrders.setOrder ) == "function" then
		local ok, result = RfsBotOrders.setOrder( unit, saved )
		if type( RfsHackApply ) == "table" and type( RfsHackApply.writePublicOrder ) == "function" then
			pcall( RfsHackApply.writePublicOrder, unit, saved )
		end
		return ok, result
	end
		...
		if isAlly then
			...
			if self.unit and self.unit.character then
				...
				if hex and hex ~= self._rfsLastTintHex then
					self._rfsLastTintHex = hex
					applyAllyVisualColor( self.unit.character, info or { allyColor = hex } )
		...
		if type( cls ) == "table" then
			if not cls._rfsHackableRpc then
				...
				function cls.sv_e_rfsIdentity( self, params )
					...
					if params and params.clear then
						...
						self.saved.friendly = false
						self.isDirty = true
						if type( RfsHackApply ) == "table" and type( RfsHackApply.wipeFalseAlly ) == "function" then
							pcall( RfsHackApply.wipeFalseAlly, self )
						end
						return
					end
				...
				function cls.sv_e_rfsRestoreAfterLoad( self, params )
					if type( RfsHackReload ) == "table" and RfsHackReload.restoreUnitSelf then
						pcall( RfsHackReload.restoreUnitSelf, self )
					elseif type( RfsHackUnitSandbox ) == "table" and RfsHackUnitSandbox.ensureDamage then
						pcall( RfsHackUnitSandbox.ensureDamage )
					end
				end
			...
			if not cls._rfsOrderRpc then
				function cls.sv_e_rfsOrder( self, params )
					...
					if self.unit and type( RfsHackApply ) == "table" and type( RfsHackApply.writePublicOrder ) == "function" then
						pcall( RfsHackApply.writePublicOrder, self.unit, self.saved.rfsOrder )
					end
				end
			...
			if type( cls.server_onUnitUpdate ) == "function" and not cls._rfsUnitHooked then
				...
				cls.server_onUnitUpdate = function( self, dt )
					self.saved = self.saved or {}
					if self.saved.playerAlly or ( self.unit and type( RfsBotHijack ) == "table"
							and RfsBotHijack.isAlly and RfsBotHijack.isAlly( self.unit ) ) then
						pcall( function()
							local now = sm.game.getCurrentTick() or 0
							local every = 40
							if type( RfsHackAllyThrottle ) == "table" and RfsHackAllyThrottle.DAMAGE_ENSURE_EVERY then
								every = tonumber( RfsHackAllyThrottle.DAMAGE_ENSURE_EVERY ) or 40
							end
							local last = tonumber( self._rfsDmgEnsureTick ) or 0
							if last == 0 or ( now - last ) >= every then
								self._rfsDmgEnsureTick = now
					...
					-- Paint gun: adopt live character color BEFORE vanilla think snaps saved.color.
					if self.unit and sm.exists( self.unit ) and ( RfsBotHijack.isAlly( self.unit ) or ( self.saved and self.saved.playerAlly ) ) then
						local liveHex = charColorHex( self.unit.character )
						local storedHex = normalizeColorHex( self.saved and self.saved.rfsAllyColor )
						if liveHex and storedHex and liveHex ~= storedHex then
							self.saved.rfsAllyColor = liveHex
							local info = RfsBotHijack.allies[unitKey( self.unit )]
							if info then
								info.allyColor = liveHex
							end
							persistAllyTintOnUnit( self, liveHex )
						end
					end
					...
					if self.unit and sm.exists( self.unit ) and ( RfsBotHijack.isAlly( self.unit ) or ( self.saved and self.saved.playerAlly ) ) then
						...
						local liveHex = charColorHex( self.unit.character )
						-- Restore stored tint if vanilla think snapped it. Paint is adopted before orig().
						if hex and liveHex ~= hex then
							local col = colorFromHex( hex )
							if col and self.unit.character and sm.exists( self.unit.character ) then
								applyColor( self.unit.character, col )
							end
							self._rfsLastTintHex = hex
						end
